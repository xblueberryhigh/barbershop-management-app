...
def create_booking(db: Session, booking: BookingCreate) -> Booking:
    ...
    check_booking_conflict(db, booking.start_time, booking.end_time)
...
def check_booking_conflict(db: Session, start_time: datetime, end_time: datetime):
    conflict = (
        db.query(Booking.id)
        .filter(Booking.start_time < end_time, Booking.end_time > start_time)
        .first()
    )
